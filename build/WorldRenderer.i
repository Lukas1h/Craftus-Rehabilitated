# 0 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
# 1 "/Users/blackout/Desktop/Craftus-Rehabilitated/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
...
# 4 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/blocks/Block.h" 2
...
# 4 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/rendering/WorldRenderer.h" 2
...
# 5 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/rendering/WorldRenderer.h" 2
...
# 8 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/rendering/Camera.h" 2
...
# 8 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/rendering/WorldRenderer.h" 2
...
# 2 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
...
# 4 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
...
# 6 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
...
void DebugUI_Draw();
# 9 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
...
# 13 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
# 1 "/Users/blackout/Desktop/Craftus-Rehabilitated/include/rendering/Hand.h" 1
...
# 14 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 2
...
static struct { RenderStep *data; int length, capacity; } renderingQueue;
static uint8_t chunkRendered[(9)][((128) / (16))][(9)];
static struct { TransparentRender *data; int length, capacity; } transparentClusters;
...
void WorldRenderer_Init(Player* player_, World* world_, WorkQueue* workqueue_, int projectionUniform_) {
 ...
 memset((&renderingQueue), 0, sizeof(*(&renderingQueue)));
 memset((&transparentClusters), 0, sizeof(*(&transparentClusters)));
 ...
 C3D_FogGasMode(GPU_FOG, GPU_PLAIN_DENSITY, 
# 75 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
                                           0
# 75 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
                                                );
...
void WorldRenderer_Deinit() {
 ( free((&renderingQueue)->data), memset((&renderingQueue), 0, sizeof(*(&renderingQueue))) );
 ( free((&transparentClusters)->data), memset((&transparentClusters), 0, sizeof(*(&transparentClusters))) );
 Cursor_Deinit();
...
static void renderWorld() {
 ...
 ((&transparentClusters)->length = 0);

 int pY = ((((WorldToChunkCoord(FastFloor(player->position.y))) > (0) ? (WorldToChunkCoord(FastFloor(player->position.y))) : (0))) < (((128) / (16)) - 1) ? (((WorldToChunkCoord(FastFloor(player->position.y))) > (0) ? (WorldToChunkCoord(FastFloor(player->position.y))) : (0))) : (((128) / (16)) - 1));
 Chunk* pChunk =
     World_GetChunk(world, WorldToChunkCoord(FastFloor(player->position.x)), WorldToChunkCoord(FastFloor(player->position.z)));
 ( vec_expand_((char**)&(&renderingQueue)->data, &(&renderingQueue)->length, &(&renderingQueue)->capacity, sizeof(*(&renderingQueue)->data)) ? -1 : ((&renderingQueue)->data[(&renderingQueue)->length++] = (((RenderStep){&pChunk->clusters[pY], pChunk, Direction_Invalid})), 0), 0 );
 chunkRendered[(9) / 2][pY][(9) / 2] = 1;

 float3 playerPos = player->position;

 while (renderingQueue.length > 0) {
  RenderStep step = (&renderingQueue)->data[--(&renderingQueue)->length];
  Chunk* chunk = step.chunk;
  Cluster* cluster = step.cluster;

  if (cluster->vertices > 0 && cluster->vbo.size) {
   chunkRendered[chunk->x - (world->cacheTranslationX - ((9) / 2))][cluster->y][chunk->z - (world->cacheTranslationZ - ((9) / 2))] |= 2;
  ...
  if (cluster->transparentVertices > 0 && cluster->transparentVBO.size) {
   ( vec_expand_((char**)&(&transparentClusters)->data, &(&transparentClusters)->length, &(&transparentClusters)->capacity, sizeof(*(&transparentClusters)->data)) ? -1 : ((&transparentClusters)->data[(&transparentClusters)->length++] = (((TransparentRender){cluster, chunk})), 0), 0 );
  }
  ...
  for (int i = 0; i < 6; i++) {
   ...
   if (newX < world->cacheTranslationX - (9) / 2 + 1 ||
       newX > world->cacheTranslationX + (9) / 2 - 1 ||
       newZ < world->cacheTranslationZ - (9) / 2 + 1 ||
       newZ > world->cacheTranslationZ + (9) / 2 - 1 || newY < 0 || newY >= ((128) / (16)))
    continue;
   float3 dist = f3_sub(f3_new(newX * (16) + (16) / 2, newY * (16) + (16) / 2,
          newZ * (16) + (16) / 2),
          playerPos);
   if (f3_dot(dist, dist) > (3.f * (16)) * (3.f * (16))) {
    continue;
   }

   if (chunkRendered[newX - (world->cacheTranslationX - ((9) / 2))][newY][newZ - (world->cacheTranslationZ - ((9) / 2))] & 1) continue;

   if (!ChunkCanBeSeenThrough(cluster->seeThrough, step.enteredFrom, i) && step.enteredFrom != Direction_Invalid)
    continue;

   C3D_FVec chunkPosition = FVec3_New(newX * (16), newY * (16), newZ * (16));
   if (!Camera_IsAABBVisible(&camera, chunkPosition, FVec3_New((16), (16), (16)))) continue;

   chunkRendered[newX - (world->cacheTranslationX - ((9) / 2))][newY][newZ - (world->cacheTranslationZ - ((9) / 2))] |= 1;
   ...
   if (newChunk) ( vec_expand_((char**)&(&renderingQueue)->data, &(&renderingQueue)->length, &(&renderingQueue)->capacity, sizeof(*(&renderingQueue)->data)) ? -1 : ((&renderingQueue)->data[(&renderingQueue)->length++] = (nextStep), 0), 0 );
  }
 }
...
# 171 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
   _Bool 
# 171 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
        clear = 
# 171 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
                1
# 171 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
    ...
    for (int xOff = -1; xOff < 2 && clear; xOff++)
     for (int zOff = -1; zOff < 2 && clear; zOff++)
      if (world->chunkCache[x + xOff][z + zOff]->genProgress == ChunkGen_Empty) clear = 
# 174 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
                                                                                       0
# 174 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
                                                                                            ;
 ...
 C3D_AlphaTest(
# 181 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
              1
# 181 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
                  , GPU_GEQUAL, 255);

 int i;
 TransparentRender* render;
 if ( (&transparentClusters)->length > 0 ) for ( (i) = (&transparentClusters)->length - 1; (i) >= 0 && (((render) = &(&transparentClusters)->data[(i)]), 1); --(i)) {
  C3D_BufInfo bufInfo;
  BufInfo_Init(&bufInfo);
 ...
 C3D_AlphaTest(
# 194 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c" 3 4
              0
# 194 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/WorldRenderer.c"
                   , GPU_GREATER, 0);

 DebugUI_Text("Clusters drawn %d with %d steps. %d vertices", clustersDrawn, steps, polysTotal);
 DebugUI_Text("T: %u P: %u %d", world->chunkCache[(9) / 2][(9) / 2]->tasksRunning,
       world->chunkCache[(9) / 2][(9) / 2]->genProgress, workqueue->queue.length);
}
