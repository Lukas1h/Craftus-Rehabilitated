# 0 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c"
# 1 "/Users/blackout/Desktop/Craftus-Rehabilitated/build//"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c"
...
# 2 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c" 2
...
# 4 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c" 2
...
# 6 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c" 2

static struct { VBO_Block *data; int length, capacity; } freedBlocks;

static LightLock lock;

void VBOCache_Init() {
 memset((&freedBlocks), 0, sizeof(*(&freedBlocks)));
 LightLock_Init(&lock);
}
void VBOCache_Deinit() {
 VBO_Block block;
 int i;
 if ( (&freedBlocks)->length > 0 ) for ( (i) = 0; (i) < (&freedBlocks)->length && (((block) = (&freedBlocks)->data[(i)]), 1); ++(i)) { linearFree(block.memory); }
 ( free((&freedBlocks)->data), memset((&freedBlocks), 0, sizeof(*(&freedBlocks))) );
}
...
VBO_Block VBO_Alloc(size_t size) {
 ...
 if (freedBlocks.length > 0) {
  VBO_Block block;
  int i;
  if ( (&freedBlocks)->length > 0 ) for ( (i) = 0; (i) < (&freedBlocks)->length && (((block) = (&freedBlocks)->data[(i)]), 1); ++(i)) {
   if (size <= block.size && block.size - size <= 2048) {
    ( vec_splice_((char**)&(&freedBlocks)->data, &(&freedBlocks)->length, &(&freedBlocks)->capacity, sizeof(*(&freedBlocks)->data), i, 1), (&freedBlocks)->length -= (1) );
...
void VBO_Free(VBO_Block block) {
 if (block.size > 0 && block.memory != 
# 45 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c" 3 4
                                      ((void *)0)
# 45 "/Users/blackout/Desktop/Craftus-Rehabilitated/source/rendering/VBOCache.c"
                                          ) {
  LightLock_Lock(&lock);
  ( vec_expand_((char**)&(&freedBlocks)->data, &(&freedBlocks)->length, &(&freedBlocks)->capacity, sizeof(*(&freedBlocks)->data)) ? -1 : ((&freedBlocks)->data[(&freedBlocks)->length++] = (block), 0), 0 );
  qsort((&freedBlocks)->data, (&freedBlocks)->length, sizeof(*(&freedBlocks)->data), &sort_by_size);
  LightLock_Unlock(&lock);
 }
}
